...
public static class CompilerUtilities
{
    ...
    public static List<string> FileFinder(in string path, in string extension) => Directory.EnumerateFiles(path, extension, SearchOption.AllDirectories).ToList();
    ...
    public static void RecordToLog(Exception ex) => Log.Fatal(ex, $" Crash! ");
    ...
    public static void CloseLog() => Log.CloseAndFlush();
}
